...
function AllCards({addProdToCart, addProdToFav }) {
...
let cards = flowers.map(flower => {
    return <CardProd name={flower.name} coast={flower.coast} img={flower.img} addProdToCart={(NEWPROD) => addProdToCart(NEWPROD)} addProdToFav={(newFav) => addProdToFav(newFav)}></CardProd>
})
